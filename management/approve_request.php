...
//select firstname, lastname and email from user id
$emailQuery="SELECT first_name, last_name, email from user WHERE user_id=$uid";
$emailSelect=$db->select($emailQuery);
foreach($emailSelect as $rowemail){
    $fname=$rowemail['first_name'];
    $lname=$rowemail['last_name'];
    $recipientEmail=$rowemail['email'];
}
...
<div class="main-content">
    ...
    <div class="page-content">
        ...
        <strong>Take Note! </strong>' . @implode('</li><li>', $error) . ' 
                                ...
                                //send email notification to all stakeholders notifying that request has been approved

                                function send_Email_All($recipientEmail, $subject, $message) {
...
$subject="Request for new equipment approved";
$message='
<!DOCTYPE html>
<html>
  <head>
            ...
            <tr>
              <td>
                ...
                <p style="font-size: 16px; color: #555;">
                  Your request for a new equipment with name <strong>'.$itemName.'</strong> and description <strong>'.$desc.'</strong> has been <strong>Approved</strong>.
                  Equipment is being procured and will be availed to you once available.
                </p>
                <br />
...
';                                
                                //send email
                                send_Email_All($recipient,$subject,$message);                                 
                                $insertQuery = "UPDATE `request` SET `status` = 'Approved' WHERE `request`.`request_id` = '".$requestid."';";
